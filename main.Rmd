...
output:
  ...
  pdf_document:
    toc: yes
subtitle: Analysis based on data from the National Oceanic and Atmospheric Administration's
  Storm Database
year: '2019'
---
...
### Get state fullnames
State names are defined in a specific CSV file [`UNITED_STATES.csv`](https://github.com/affagard/RepData_PeerAssessment2/blob/master/data/UNITED_STATES.csv), dowloadable [here [638 bytes]](https://github.com/affagard/RepData_PeerAssessment2/blob/master/data/UNITED_STATES.csv)
...
```{r Regular Event types, message=FALSE, warning=FALSE, cache=TRUE}
...
g <- g + coord_flip(xlim = NULL, ylim = NULL, expand = TRUE, clip = "on")
g <- g + scale_y_continuous(breaks=seq(0,max(storm_data.material.damages$damages_in_doll/1000000000)+200,10), position = "right", limits = c(0,NA), expand = c(0, 1))
